...
--
-- The pilot has measured the stylist side since 0029 - looks composed, looks
...
-- A client therefore cannot write events onto another member record.
--
-- Chloe own browsing is not data. The admin previews the client area in HER
...
--
-- NOTE ON STYLE: this file is deliberately plain ASCII with no apostrophes in
-- its comments, and it ends on a terminated statement rather than a comment.
-- It is meant to be pasted whole into the Supabase SQL editor, and those are
-- the three things that most often survive badly in a copy and paste.
...
-- ---------------------------------------------------------------------------
-- event_id is an identity column rather than a uuid because this is the one
-- table whose natural order matters: the replay plays events back in the order
-- they were recorded, and a random uuid carries no order at all.
create table if not exists public.client_event (
  ...
  room       text,
  path       text,
  -- What it reads as in the replay: SENT LOOK 3, BURBERRY TRENCH, NET-A-PORTER.
  label      text,
...
-- No anon or authenticated policies on purpose, exactly as 0029: RLS on with
-- zero policies means only the service-role admin client can read or write
-- these. The client area writes through the ingest route, which uses that
-- client after checking the signed-in user; the admin reads through server
-- actions. This is the last statement in the file on purpose.
