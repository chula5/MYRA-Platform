          ...
          <div>
              ...
              {BRAND_GROUPS.map((g) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className={`text-[10px] tracking-[0.08em] mb-3 ${active ? 'text-white/70' : 'text-[#A8A8A4]'}`}>
                      {g.blurb}
                    </p>
                    <p className={`text-[9px] tracking-[0.10em] leading-relaxed ${active ? 'text-white/60' : 'text-[#6B6B6B]'}`}>
                      {g.brands.join(' · ')}
                    </p>
                  </button>
                )
            ...
            <div className="flex justify-center">
              <button
                ...
                onClick={() => setStep(2)}
                className="bg-[#0A0A0A] text-white px-12 py-3.5 rounded-[3px] text-[11px] tracking-[0.22em] hover:opacity-85 transition-opacity disabled:opacity-25"
              >
                CONTINUE →
              </button>
            </div>
            {brandGroups.length === 0 && (
              <p className="text-center text-[9px] tracking-[0.16em] text-[#A8A8A4] mt-4">
          ...
          <div>
            ...
            {ratingOutfits.length === 0 ? (
              <div className="text-center py-16">
                <p className="text-[11px] tracking-[0.20em] text-[#A8A8A4] mb-8">
                  NO OUTFITS TO RATE RIGHT NOW
                </p>
                <button
                  onClick={() => finish(liked, disliked)}
                  disabled={submitting}
                  className="bg-[#0A0A0A] text-white px-12 py-3.5 rounded-[3px] text-[11px] tracking-[0.22em] hover:opacity-85 transition-opacity disabled:opacity-50"
                >
              ...
              <>
                {/* Progress */}
                <p className="text-center text-[10px] tracking-[0.20em] text-[#6B6B6B] mb-4">
                ...
                <div className="relative max-w-[340px] mx-auto mb-8" style={{ height: '454px' }}>
                  {ratingOutfits[cardIdx] && (
                    ...
                    >
                      ...
                      {/* Swipe hint overlays */}
                      {dragX > 30 && (
                        <div className="absolute top-5 left-5 border-2 border-[#3A6B3A] text-[#3A6B3A] px-3 py-1 rounded text-[12px] tracking-[0.18em] rotate-[-12deg] bg-white/80">
                          LOVE
                        </div>
                      )}
                      {dragX < -30 && (
                        <div className="absolute top-5 right-5 border-2 border-[#B83A3A] text-[#B83A3A] px-3 py-1 rounded text-[12px] tracking-[0.18em] rotate-[12deg] bg-white/80">
                          PASS
                        </div>
                      )}
                    </div>
