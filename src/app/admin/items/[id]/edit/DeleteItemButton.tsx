    ...
    <div>
      ...
      {!confirming ? (
        <button
          ...
          className="w-full border border-[#E8B4B4] text-[#B83A3A] px-4 py-2.5 text-[10px] tracking-[0.20em] hover:bg-[#B83A3A] hover:text-white hover:border-[#B83A3A] transition-colors duration-300"
        >
          DELETE ITEM
        </button>
      ) : (
        <div className="border border-[#E8B4B4] bg-[#FDECEC] p-3 rounded-[3px]">
          <p className="text-[10px] tracking-[0.12em] text-[#B83A3A] leading-relaxed mb-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={doDelete}
              disabled={busy}
              className="flex-1 bg-[#B83A3A] text-white px-3 py-2 text-[10px] tracking-[0.18em] hover:bg-[#9c2f2f] transition-colors disabled:opacity-50"
            >
      ...
      {error && <p className="mt-2 text-[9px] tracking-[0.12em] text-[#B83A3A]">{error.toUpperCase()}</p>}
    </div>
  )
}
