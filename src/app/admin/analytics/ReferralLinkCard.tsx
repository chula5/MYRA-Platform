  ...
  return (
    <div className="border border-[#E2E0DB] bg-white rounded-[3px] p-4 flex items-center justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className="text-[12px] tracking-[0.04em] text-[#4A4E57] truncate font-mono">{url}</p>
      </div>
      <button
        onClick={copy}
        className="flex-shrink-0 border border-[#0A0A0A] text-[#4A4E57] px-4 py-2 text-[9px] tracking-[0.18em] hover:bg-[#0A0A0A] hover:text-white transition-colors"
      >
        {copied ? 'COPIED ✓' : 'COPY'}
      </button>
    </div>
