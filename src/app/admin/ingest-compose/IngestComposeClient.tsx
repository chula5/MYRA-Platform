...
type Block = IngestComposeResult & { states: Record<number, CandState> }
...
export default function IngestComposeClient() {
  ...
  async function run() {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      setBlocks((prev) => [...prev, { ...res, states: {} }])
  ...
  async function approve(bi: number, ci: number) {
    ...
    const cand = block.candidates?.[ci]
      ...
      <div className="border border-[#E2E0DB] bg-white rounded-[12px] p-6 mb-10 max-w-[760px]">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="bg-[#0A0A0A] text-white rounded-full px-7 py-3 text-[10px] tracking-[0.16em] hover:opacity-85 transition-opacity disabled:opacity-40"
          >
            {running ? 'ADDING & COMPOSING…' : 'ADD & COMPOSE'}
          </button>
          {progress && (
            <span className="text-[10px] tracking-[0.09em] text-[#A8A8A4]">{progress.done} / {progress.total} DONE</span>
          )}
        </div>
        <p className="text-[9px] tracking-[0.054em] text-[#A8A8A4] mt-3 leading-relaxed">
          ...
          library. Tap YES to save a look as a draft (auto-triggers a Higgsfield shoot &amp; feeds the Style Brain).
        </p>
      </div>

      {/* Results — one section per ingested item */}
          ...
          <div key={bi} className="border border-[#E2E0DB] bg-white rounded-[14px] overflow-hidden">
            ...
            <div className="p-4">
              ...
              {!block.error && (!block.candidates || block.candidates.length === 0) && (
                <p className="text-[10px] tracking-[0.14em] text-[#A8A8A4] py-4 text-center">
                  ITEM SAVED — NO BRAND-COHERENT COMBINATIONS FOUND YET. ADD MORE ITEMS TO PAIR IT WITH.
                </p>
              )}
                  ...
                  return (
                    <div key={ci} className="border border-[#E2E0DB] rounded-[10px] p-3">
                      ...
                      <div className="grid grid-cols-4 gap-1.5 mb-3">
                        ...
                        {c.items.map((item, k) => (
                          <div key={`${item.item_id}-${k}`} className="relative">
                            <div className="aspect-[3/4] rounded-[6px] overflow-hidden bg-[#F2F2F0]">
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                      ...
                      {st?.approved ? (
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => approve(bi, ci)} disabled={st?.approving} className="flex-1 bg-[#0A0A0A] text-white py-2.5 text-[10px] tracking-[0.18em] rounded-full hover:opacity-85 transition-opacity disabled:opacity-50">
                            {st?.approving ? 'CREATING…' : 'YES ✓'}
                          </button>
                          <button onClick={() => setState(bi, ci, { discarded: true })} disabled={st?.approving} className="px-4 py-2.5 text-[10px] tracking-[0.14em] border border-[#E2E0DB] text-[#6B6B6B] rounded-full hover:border-[#0A0A0A] transition-colors disabled:opacity-50">
                            SKIP
                          </button>
                        </div>
                      )}
