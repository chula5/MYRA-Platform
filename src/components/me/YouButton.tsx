...
export default function YouButton({ active, onClick }: { active?: boolean; onClick?: () => void }) {
  const inner = (
    <>
      <span className={`block w-[42px] h-[42px] mx-auto ${active ? 'text-[#2B2B2B]' : 'text-[#55534E] group-hover:text-[#2B2B2B]'}`}>
      ...
      <span className={`block mt-1 text-[18px] ${active ? 'text-[#2B2B2B]' : 'text-[#55534E] group-hover:text-[#2B2B2B]'}`}>You</span>
    </>
  )
  const cls = 'group text-center shrink-0'
  return onClick
    ? <button type="button" onClick={onClick} className={cls}>{inner}</button>
    : <Link href="/me/profile" className={cls}>{inner}</Link>
}
