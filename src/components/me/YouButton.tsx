...
export default function YouButton({ active, onClick }: { active?: boolean; onClick?: () => void }) {
  const inner = (
    <>
      <span className={`block w-[46px] h-[46px] mx-auto ${active ? 'text-[#2B2B2B]' : 'text-[#55534E] group-hover:text-[#2B2B2B]'}`}>
      ...
      <span className={`block mt-1 text-[20px] ${active ? 'text-[#2B2B2B]' : 'text-[#55534E] group-hover:text-[#2B2B2B]'}`}>You</span>
    </>
  )
