...
export default function YouButton({ active, onClick }: { active?: boolean; onClick?: () => void }) {
  const inner = (
    <>
      <span className={`block w-[clamp(54px,2.9vw,104px)] h-[clamp(54px,2.9vw,104px)] mx-auto ${active ? 'text-[#2B2B2B]' : 'text-[#55534E] group-hover:text-[#2B2B2B]'}`}>
      ...
      <span className={`block mt-1 text-[clamp(21px,1.15vw,40px)] ${active ? 'text-[#2B2B2B]' : 'text-[#55534E] group-hover:text-[#2B2B2B]'}`}>You</span>
    </>
  )
