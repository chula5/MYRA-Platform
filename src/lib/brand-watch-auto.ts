...
//
// Chloe switches AUTOMATE on per brand once its trust (brand-watch-trust) is
// earned. From then, after each scan, the brand's NEWLY discovered queued
// pieces that the learning would keep go straight to the library as ready —
// the same keep as her KEEP button, marked auto-kept. Everything else stays in
// the queue for her.
//
// Guard rails:
//  · only pieces discovered after AUTOMATE was switched on — never the backlog
//  · trust is re-measured at every scan; if it has slipped, nothing is kept and
//    the scan says AUTOMATE PAUSED
...
async function loadDecisions(admin: any): Promise<any[]> {
...
/** After a scan: keep the brand's new pieces the learning would keep — if AUTOMATE is on and still earned. */
export async function autoKeepForBrand(admin: any, watched: WatchedBrandRow, trustData?: BrandTrustData): Promise<AutoKeepResult | null> {
  if (!watched.auto_keep || !watched.brand_id) return null
  const data = trustData ?? (await loadBrandTrust(admin))
  const trust = trustFor(data, watched)
  if (!trust.trusted) return { autoKept: 0, note: `AUTOMATE PAUSED — ${trust.summary}` }

  const since = watched.auto_keep_since ?? new Date().toISOString()
  const { data: queued, error } = await admin.from('brand_watch_queue')
    .select('queue_id, product_name, item_type, colour_family, material_category, material_primary, price, price_gbp, discovery_score')
    .eq('status', 'queued').eq('brand_id', watched.brand_id).gte('discovered_at', since)
    .limit(1000)
  if (error) return { autoKept: 0, note: `AUTOMATE FAILED — ${error.message}` }

  const picks = ((queued ?? []) as any[])
    .filter((q) => q.item_type && !houseBanOf({ title: q.product_name, materialPrimary: q.material_primary, itemType: q.item_type }))
    .map((q) => ({ q, delta: data.learn(toDecided({ ...q, brand_id: watched.brand_id })).delta, score: Number(q.discovery_score ?? 0) }))
    .filter(({ delta, score }) => wouldAutoKeep(delta, score, watched.min_score))
    .sort((a, b) => (b.score + b.delta) - (a.score + a.delta))
    .slice(0, AUTO_KEEP_PER_SCAN)

  if (!picks.length) return { autoKept: 0, note: null }
  const autoKept = await keepQueueRows(admin, picks.map((p) => p.q.queue_id), { auto: true })
  return { autoKept, note: null }
}
