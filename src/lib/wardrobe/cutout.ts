...
// sharp is loaded lazily, the way cloudinary-persist.ts does it — a static
// import makes Next's dev bundler look for sharp/index.js and 500 the route.
type SharpFn = (typeof import('sharp'))['default']
let sharpMod: SharpFn | null = null
async function sharp(): Promise<SharpFn> {
...
export async function normalisePhoto(bytes: Buffer, maxEdge = 2048): Promise<{ png: Buffer; width: number; height: number }> {
  const png = await (await sharp())(bytes).rotate().resize(maxEdge, maxEdge, { fit: 'inside', withoutEnlargement: true }).toColorspace('srgb').png().toBuffer()
  const meta = await (await sharp())(png).metadata()
...
export async function detectorJpeg(bytes: Buffer, maxEdge = 1536): Promise<Buffer> {
  return (await sharp())(bytes).rotate().resize(maxEdge, maxEdge, { fit: 'inside', withoutEnlargement: true }).jpeg({ quality: 88 }).toBuffer()
}
...
export async function cropGarment(normalisedPng: Buffer, box: DetectedGarment['bounding_box'], pad = 0.1): Promise<Buffer> {
  const meta = await (await sharp())(normalisedPng).metadata()
  ...
  return (await sharp())(normalisedPng)
...
export async function frameOnWhite(pngBytes: Buffer, width = 1024, height = 1365, occupancy = 0.86): Promise<Buffer> {
  const flat = await (await sharp())(pngBytes).flatten({ background: '#ffffff' }).png().toBuffer()
  // Trim the white margin the model added (tolerant threshold), then re-pad.
  let trimmed = flat
  try {
    trimmed = await (await sharp())(flat).trim({ background: '#ffffff', threshold: 18 }).png().toBuffer()
  } catch { /* nothing to trim */ }
  const target = Math.round(width * occupancy)
  const resized = await (await sharp())(trimmed)
...
export async function cutoutLooksValid(pngBytes: Buffer): Promise<{ ok: boolean; reason?: string }> {
  const { data, info } = await (await sharp())(pngBytes).flatten({ background: '#ffffff' }).raw().toBuffer({ resolveWithObject: true })
  const W = info.width
  const H = info.height
